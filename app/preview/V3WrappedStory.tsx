...
function GuessQuestionScreen({
  ...
  const palette = V3_CARD_PALETTES[beatIndex % V3_CARD_PALETTES.length];
...
function RyanQuipBubble({ text, isActive }: { text: string; isActive: boolean }) {
  const [visible, setVisible] = useState(false);
  const [displayed, setDisplayed] = useState("");
  const posRef = useRef(0);
  const hasTriggered = useRef(false);

  // Only trigger when the screen becomes active
  useEffect(() => {
...
function CardRevealScreen({
  ...
  const data = V3_BEAT_DATA[beatId];
  if (!data) return null;
  const palette = V3_CARD_PALETTES[beatIndex % V3_CARD_PALETTES.length];
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1, marginBottom: SPACE_L }}>
        <RyanQuipBubble text={ryan} isActive={isActive} />
      </div>
...
export default function V3WrappedStory({ onClose, startAtBeat }: { onClose: () => void; startAtBeat?: number }) {
  ...
  const bgColor = isQuestion
    ? BG_PRIMARY
    : V3_CARD_PALETTES[currentScreen?.beatIndex % V3_CARD_PALETTES.length]?.bg ?? BG_PRIMARY;
    ...
    <div className="relative h-full w-full" style={{ backgroundColor: bgColor, overflow: "hidden", transition: `background-color ${DURATION}ms ${EASE}` }}>
      ...
      >
        {screens.map((screen, i) => (
          ...
          >
            {Math.abs(i - index) <= 1 && renderScreen(screen, advance, onClose, i === index)}
          </div>
        ))}
      </div>
      ...
      {!isQuestion && (
        ...
        >
          <RoundButton ariaLabel={isClose ? "Done" : "Next"} onClick={isClose ? onClose : advance}>
            <ForwardIcon />
          </RoundButton>
        </div>
      )}
...
function renderScreen(screen: Screen, advance: () => void, close: () => void, isActive: boolean) {
